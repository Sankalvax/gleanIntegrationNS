...
const IndexUsersPage = () => {
    ...
    const handleIndexUsersClick = async () => {
        ...
        try {
            const response = await fetch('http://localhost:5001/api/bulk_doc_index', { // Ensure Flask is on port 5001
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                ...
                setApiMessage('Document Indexing process completed successfully!');
         
                // alert(`✅ Users indexing process status: ${result.message}`);
            } else {
                console.error("API Error:", result);
                setApiError(result.error || `Failed Document Indexing : ${response.statusText}`);
        ...
        <div className="flex flex-col justify-center items-center min-h-screen bg-gray-50 px-6 py-12">
            <div className="w-full max-w-3xl bg-white rounded-2xl shadow-lg border overflow-hidden p-8">
                <div className="flex justify-start items-center mb-6">
                    ...
                    <h1 className="text-2xl font-semibold text-gray-800 ml-4">Step 3: Bulk Document Indexing</h1>
                </div>

                <div className="mb-6 text-left">
                    <h3 className="text-lg font-semibold text-gray-700 mb-2">Objects to be Indexed:</h3>
